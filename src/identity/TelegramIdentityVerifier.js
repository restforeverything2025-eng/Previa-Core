...
class TelegramIdentityVerifier {
    ...
    verify(initData, nowSeconds = Math.floor(Date.now() / 1000)) {
        if (typeof initData !== "string" || !initData.trim()) {
            throw fail("Telegram initData is required.");
        }
        ...
        try {
            ...
            throw fail("Telegram initData is invalid.");
        }
        ...
        if (!receivedHash || !authDateRaw || !userRaw) {
            throw fail("Telegram initData is missing required fields.");
        }

        const authDate = Number(authDateRaw);
        if (!Number.isInteger(authDate)) {
            throw fail("Telegram auth_date is invalid.");
        }

        const age = nowSeconds - authDate;
        if (age < 0 || age > this.maxAgeSeconds) {
            throw fail("Telegram initData has expired or is not yet valid.");
        }
        ...
        if (!safeHashEqual(receivedHash, expectedHash)) {
            throw fail("Telegram initData signature is invalid.");
        }
        ...
        try {
            ...
            throw fail("Telegram user data is invalid.");
        }

        if (!user || user.id === undefined || user.id === null) {
            throw fail("Telegram user id is missing.");
        }
