 ...
 *
 * Telegram identity is verified here before trusted identity data
 * is passed into the application endpoint.
 * ============================================================
 */
...
class OrderHttpHandler {
  ...
  async create(request = {}) {
    ...
    try {
      if (!this.identityVerifier) {
        throw Object.assign(
          new Error("Identity verifier is not configured"),
          { code: "INTERNAL_ERROR", retryable: false }
        );
      }

      const identity = this.identityVerifier.verify(
      ...
      return {
        status: this._statusForResult(result),
        body: result
      };
    } catch (error) {
  ...
  _statusForResult(result) {
    return result.success ? 200 : this._statusForCode(result.code);
  }
